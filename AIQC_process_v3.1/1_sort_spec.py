...
#%% if don't print error message on the screen
    if first_P==0 and num!=0:
        print(target+" can't find first data")
        no_first_data_log=open(opath+'no_first_data_list.txt','a',newline='\n')
        no_first_data_log.write(target+" can't find first data"+'\n')
...
#%% output data
    ...
    filename='no_'+ST_no+launchtime.strftime('_%Y%m%d_%H%M')
    opdata.to_csv(opath+filename+'_L1.csv',index=False)

#%% log file
    log_file=open(opath+filename+'_L1_log.txt','w',newline='\n')
